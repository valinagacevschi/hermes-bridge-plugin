...
plugins/platforms/hermes_bridge/test_attachments.py — edit the source there, not here.
...
class TestSendVoice(unittest.IsolatedAsyncioTestCase):
    """Outbound voice/TTS replies (#38) — same _send_attachment path, audio mime."""
...
class TestInboundAttachments(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_receive_loop_sets_voice_message_type_for_audio(self):
        """#38: inbound audio must set message_type=VOICE (not just an audio
        mime in media_types) — gateway/run.py's /voice `voice_only` auto-TTS
        mode keys directly off event.message_type, not per-attachment mime."""
        from hermes_bridge.crypto import seal
    ...
    async def test_receive_loop_defaults_to_text_message_type_with_no_media(self):
        ...
        the default MessageType.TEXT — added alongside #38's message_type
        classification so a bug there can't silently reclassify every
